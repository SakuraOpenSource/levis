...
func NewWithCaptchaStore(rt *runtime.Runtime, plugins *plugin.Manager, debug bool, store service.CaptchaStore) (*gin.Engine, func()) {
	...
	// 余额全额结清账单（订单账单走开通，续费账单顺延到期）。
	invoices.POST("/:id/pay", h.PayInvoice)
	...
	admin.POST("/users/batch-status", h.AdminBatchUsersStatus)
	admin.POST("/users/batch-delete", h.AdminBatchDeleteUsers)
	...
	admin.POST("/products/batch-status", h.AdminBatchProductsStatus)
	admin.POST("/products/batch-delete", h.AdminBatchDeleteProducts)
	...
	admin.GET("/orders/:id", h.AdminOrder)
	admin.GET("/invoices", h.AdminInvoices)
	admin.GET("/invoices/:id", h.AdminInvoice)
	admin.GET("/services", h.AdminServices)
	admin.PATCH("/services/:id", h.AdminUpdateService)
	admin.POST("/services/:id/retry", h.AdminRetryService)
	admin.POST("/services/:id/bind", h.AdminBindService)
	admin.DELETE("/services/:id", h.AdminDeleteService)
	admin.POST("/services/batch-status", h.AdminBatchServicesStatus)
	admin.POST("/services/batch-delete", h.AdminBatchDeleteServices)
